"""
LSTM Model for Hate Speech Classification - PyTorch Implementation
Long Short-Term Memory network for sequential text processing
...
from config import (
    LSTM_CONFIG, NUM_CLASSES, MODEL_FILES,
...
class LSTMModel:
    """LSTM-based text classifier for hate speech detection."""
    ...
    def save(self, filepath: str = None):
        ...
        filepath = filepath or str(MODEL_FILES['lstm']).replace('.keras', '.pt')
        torch.save({
    ...
    @staticmethod
    def load(filepath: str = None) -> 'LSTMModel':
        """Load model from disk."""
        filepath = filepath or str(MODEL_FILES['lstm']).replace('.keras', '.pt')
        checkpoint = torch.load(filepath, map_location='cpu')
        
        lstm_model = LSTMModel(config=checkpoint['config'])
        lstm_model.build_model()
        lstm_model.model.load_state_dict(checkpoint['model_state_dict'])
        lstm_model.history = checkpoint.get('history')
        lstm_model.training_time = checkpoint.get('training_time')
    ...
    def get_model_info(self) -> Dict:
        ...
        if self.model is None:
            return {'model_type': 'LSTM', 'built': False}
    ...
    def __repr__(self):
        if self.model is None:
            return "LSTMModel(not built)"
        else:
            params = sum(p.numel() for p in self.model.parameters())
            return f"LSTMModel(params={params:,}, lstm_units={self.lstm_units}, device={self.device})"


def create_lstm_model(config: Dict = None) -> LSTMModel:
